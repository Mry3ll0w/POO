...
Cadena Numero::espacio(const Cadena &cad)
{
    ...
    std::remove_if(aux_cad.begin(), aux_cad.end() + 1, [](char c)
                   { return isspace(c); });
...
ostream &operator<<(std::ostream &salida, const Tarjeta::Tipo a) noexcept
{
    ...
    case Tarjeta::AmericanExpress:
        salida << "AmericanExpress";
        break;
    case Tarjeta::Otro:
        salida << "Tipo indeterminado";
        break;
    }
    return salida;
}
...
ostream &operator<<(std::ostream &salida, const Tarjeta &t) noexcept
{
    ...
    {
        if (islower(aux_cad_temp[i]))
            aux_cad_temp[i] = toupper(aux_cad_temp[i]);
        i++;
    }
    salida << aux_cad_temp << " ";
    i = 0;
